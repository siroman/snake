...
function drawGrid() {
  ...
  for (let x = 0; x < grid.size.width; x += 1) for (let y = 0; y < grid.size.height; y += 1) rect(grid.size.pixelX * x, grid.size.pixelY * y, grid.size.pixelX, grid.size.pixelY);
}

function keyPressed() {
  if (key.startsWith('Arrow')) snake.direction = key.substring(5).toLowerCase();
}
